...
@CrossOrigin("*")
public class SubdivAttachmentController {
    private final SubDivService subdivService;
    private final AttachmentService attachmentService;

    public SubdivAttachmentController(SubDivService subDivService, 
    ...
    public ResponseEntity<?> getRequestAttachment(@PathVariable Long id){
        Optional<PDFAttachment> attachment = subdivService.getSubdivRequestAttachment(id);
    ...
    public ResponseEntity<?> getApprovalAttachment(@PathVariable Long id){
        ...
        Optional<PDFAttachment> attachment = subdivService.getSubdivApprovalAttachment(id);
        if(attachment.isPresent()){
            return ResponseEntity.ok().body(attachment);
        }
        return ResponseEntity.noContent().build();
    ...
    public ResponseEntity<Resource> downloadProcurementAttachment(@PathVariable Long fileId){
        ...
        Resource resource = subdivService.downloadSubdivAttachment(fileId);
    ...
    public ResponseEntity<?> uploadRequestAttachment(@PathVariable Long id, @RequestParam("file") MultipartFile file){
        try{
            PDFAttachment savedAttachment = subdivService.uploadRequestAttachment(file, id);
    ...
    public ResponseEntity<?> deleteRequestAttachment(@PathVariable Long fileId){
        subdivService.deleteRequestAttachment(fileId);
